...
from DASHBOARDS.ISEE import CFG_DASH_BLOB as CFG_DASHBOARD
import numpy as np
import tempfile
...
def read_from_blob(filepath, container_client):
    filepath = filepath.replace('\\', '/')
    print(filepath)
...
def yearly_timeseries_data_prep(ts_code, unique_pi_module_name, folder_raw, PI_code, plans_selected, Baseline, Region,
                                start_year, end_year, Variable, CFG_DASHBOARD, LakeSL_prob_1D, container_client):
    ...
    for p in plans_all:
        ...
        for s in sect:
            ...
            if feather_name not in feather_done:
                ...
                df = read_from_blob(filepath, container_client)
